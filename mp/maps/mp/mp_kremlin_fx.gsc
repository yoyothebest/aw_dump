...
/#
    if ( getdvar( "clientSideEffects" ) != "1" )
        maps\createfx\mp_kremlin_fx::main();
#/
...
laser_grid_a_fx()
{
	...
	// angles and position from old exploder	
	fx_angles = ( 360, 359, 0 );
	fx_forward = AnglesToForward( fx_angles );
	fx_up = AnglesToUp( fx_angles );
	laser_grid_a_ent = SpawnFx( getfx( "krem_laser_patch_a" ), (-61, 834, 139), fx_forward, fx_up );
	TriggerFX( laser_grid_a_ent );
	
	level waittill( "minefield_complete" );
	laser_grid_a_ent Delete();
}
...
laser_grid_b_fx()
{
	...
	fx_forward = AnglesToForward( fx_angles );
	fx_up = AnglesToUp( fx_angles );
	laser_grid_b_ent = SpawnFx( getfx( "krem_laser_patch_b" ), (-353, -706, 156), fx_forward, fx_up );
	TriggerFX( laser_grid_b_ent );
	
	level waittill( "minefield_complete" );
	laser_grid_b_ent Delete();
}
...
{
	wait 1;
	PlayFXOnTag(getFX("krm_mine_snow_puff"), self, "tag_origin" );
}
