...
main()
{
	...
	//level._effect[ "screen_overlay_frosted" ] = loadfx( "vfx/ui/screen_overlay_frosted" );
	...
	//Exploder
	//level._effect[ "mp_torqued_event_xsim" ] 					= loadfx( "vfx/map/mp_torqued/mp_torqued_event_xsim" );	
	...
	/#
	    ...
	    {
	        maps\createfx\mp_torqued_fx::main();
//	        maps\mp\mp_torqued::ScriptableLightStateChange();
	    }
	#/
	    
//	    PreCacheShader ("screen_frosted_overlay");
	if (level.nextgen)
	{
...
//    { 
//		player CreateScreenFrost();
//	}
		
	avalanchetime = (level.DynEventAvalanche.AvalanchePhase1Time+level.DynEventAvalanche.AvalanchePhase2Time+level.DynEventAvalanche.AvalanchePhase3Time);
	waitframe();
...
avalanche_fx()
{
	...
	//waittime = (level.DynEventAvalanche.AvalanchePhase1Time+level.DynEventAvalanche.AvalanchePhase2Time);
	waitframe();
...
avalanche_impact_fx()
{
	...
	//waittime = (level.DynEventAvalanche.AvalanchePhase1Time+level.DynEventAvalanche.AvalanchePhase2Time+2);
	wait (9);
	ActivateClientExploder(204);
	//wait(level.DynEventAvalanche.AvalanchePhase1Time+level.DynEventAvalanche.AvalanchePhase2Time+1);
	wait (3);
...
//{
//	
//	waittime = (level.DynEventAvalanche.AvalanchePhase1Time);
//	wait (waittime);
//	ActivateClientExploder(208);
//
//}


//hides broken windows at start of level
window_broken_hide_fx()
{
	waitframe();
	windows_broken = GetEntArray("window_broken", "targetname");
	foreach (window in windows_broken)
	{	
		window hide();
	}
}
...
window_swap_fx()
{
	waitframe();
	//windows_broken = GetEntArray("window_broken", "targetname");
	windows_intact = GetEntArray("window_intact", "targetname");
	
	ActivateClientExploder(209);
	
	foreach (window in windows_intact)
	{
		
		window thread hide_window_script_delay();
	}
	
//	foreach (window in windows_broken)
//	{	
//		window thread show_window_script_delay();
//	}
...
{
	wait (0.25);
	coffee_windows_back_array=getglassarray("coffee_windows_back");
		foreach (window in coffee_windows_back_array)
			{
			center_pos = (-1115, 478, 1059);
			glass_pos = GetGlassOrigin(window);
			direction = (glass_pos-center_pos);
			DestroyGlass(window,direction);
			}
}
...
{
	wait (10.5);
	coffee_windows_front_array=getglassarray("coffee_windows_front");
		foreach (window in coffee_windows_front_array)
			{
			center_pos = (-1124, -91, 951);
			glass_pos = GetGlassOrigin(window);
			direction = (glass_pos-center_pos);
			DestroyGlass(window,direction);
			}
...
{
	wait (10.6);
	trq_windows_front_array=getglassarray("trq_windows_front");
		foreach (window in trq_windows_front_array)
			{
			center_pos = (-602, 158, 930);
			glass_pos = GetGlassOrigin(window);
			direction = (glass_pos-center_pos);
			DestroyGlass(window,direction);
			}
...
{
	wait (0.4);
	boardstore_windows_back_array=getglassarray("boardstore_windows_back");
		foreach (window in boardstore_windows_back_array)
			{
			center_pos = (487, 253, 1143);
			glass_pos = GetGlassOrigin(window);
			direction = (glass_pos-center_pos);
			DestroyGlass(window,direction);
			}
...
{
	wait (10.9);
	boardstore_windows_front_array=getglassarray("boardstore_windows_front");
		foreach (window in boardstore_windows_front_array)
			{
			center_pos = (400, 128, 1060);
			glass_pos = GetGlassOrigin(window);
			direction = (glass_pos-center_pos);
			DestroyGlass(window,direction);
			}
...
{
	wait (12.2);
	lodge_windows_front_array=getglassarray("lodge_windows_front");
		foreach (window in lodge_windows_front_array)
			{
			center_pos = (1566, 243, 947);
			glass_pos = GetGlassOrigin(window);
			direction = (glass_pos-center_pos);
			DestroyGlass(window,direction);
			}
...
//{
//    // TODO: Cleanup HUD
//    if ( !IsDefined( self.ScreenFrostOverlay ) )
//    {
//        self.ScreenFrostOverlay = newClientHudElem( self );
//        self.ScreenFrostOverlay.x = 0;
//        self.ScreenFrostOverlay.y = 0;
//        self.ScreenFrostOverlay setshader( "screen_frosted_overlay", 640, 480 );
//        self.ScreenFrostOverlay.alignX = "left";
//        self.ScreenFrostOverlay.alignY = "top";
//        self.ScreenFrostOverlay.horzAlign = "fullscreen";
//        self.ScreenFrostOverlay.vertAlign = "fullscreen";
//        self.ScreenFrostOverlay.alpha = 1;
//    }
//} 
